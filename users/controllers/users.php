	...
	public function login()
	{
		...
		if ($this->auth->is_logged_in() === FALSE)
		{
			if (isset($_POST['log-me-in']))
			{
				...
				// Try to login
                                $ga_status = $this->auth->is_google_authenticator_enabled($this->input->post('login'));
				if ($this->auth->login($this->input->post('login'), $this->input->post('password'), $remember, $ga_status) === TRUE)
				{
                                        //Is Google authentication enabled?
                                        $this->load->library('users/g_auth');
                                        if($this->auth->is_google_authenticator_enabled($this->input->post('login')) === TRUE)
                                        {
//                                            $this->user_model->update_google_authenticator_salt($this->session->userdata('id'));
//                                            log_activity($this->session->userdata('id'), lang('us_google_log_logged') . ': ' . $this->input->ip_address(), 'users');
                                            Template::redirect('users/login_ga');
                                        }
	...
	public function profile()
	{
		...
		if (isset($_POST['save']))
		{
			...
			else
			{
                ...
                if (isset($_POST['google_authentication_add']))
		{
                    Template::redirect('/users/add_google_auth');
                }
                if (isset($_POST['google_authentication_remove']))
		{
                    Template::redirect('/users/remove_google_auth');
                }
		...
		Template::set('languages', unserialize($this->settings_lib->item('site.languages')));
                ...
                Template::set('google_auth_status', $this->current_user->ga_enabled);
	...
	/**
	 * Adds google authentication to a users accont
         * 
	 *
	 * @access public
	 *
	...
	public function add_google_auth()
	{
            $this->auth->restrict();
            $this->set_current_user();
            if($this->current_user->ga_enabled == 1)
            {
                Template::set_message(lang('us_google_auth_already_enabled'),'info');
                Template::redirect('users/profile');
            }
            $this->load->library('users/g_auth');
            if($this->input->post('submit_ga'))
            {
                ...
                if ($this->form_validation->run())
		{
                    ...
                    {
                        if($this->user_model->set_google_auth_enabled($this->current_user->id, TRUE) == TRUE)
                        {
                            Template::set_message(lang('us_google_auth_verify_success'),'success');
                            Template::redirect('users/profile');
                        }
                    }
                    Template::set_message(lang('us_google_auth_verify_error'));
                }
            }
            $this->lang->load('settings');
            $salt =$this->user_model->set_google_auth_seed($this->current_user->id);
            //Spaces are removed from site name as qr code generator doesn't like it!
            $ga_qr_url = $this->g_auth->get_qr_url_code($this->current_user->username,  str_replace(' ', '', $this->settings_lib->item('site.title')), $salt);
            Template::set('ga_qr_url', $ga_qr_url);
            Template::set('salt', $salt);
            Template::render();
	}//end  add_google_auth()                
	...
	/**
	 * Removes google authentication from users accont
	 *
	 * @access public
	 *
	 * @return void
	 */
	public function remove_google_auth()
	{
            $this->auth->restrict();
            $this->set_current_user();
            $this->load->library('users/g_auth');
                ...
                if ($this->form_validation->run())
		{
                    if($this->g_auth->verify_key($this->current_user->ga_salt, $this->input->post('ga_code')))
                    {
                        if($this->user_model->set_google_auth_enabled($this->current_user->id, FALSE) == TRUE)
                        {
                            ...
                            Template::set_message(lang('us_google_auth_removal_success'),'success');
                            Template::redirect('users/profile');
                        }
                    }
                    Template::set_message(lang('us_google_auth_removal_error'));
                }
            }
            Template::render();
	}//end  remove_google_auth()                

        //--------------------------------------------------------------------

	/**
	 * Complete login if they have Google Authenticator enabled
	 *
	 * @access public
	 *
	 * @return void
	...
	public function login_ga()
        {
            ...
            if ($this->auth->is_logged_in() === FALSE)
            {
                if (isset($_POST['ga-log-me-in']))
                {
                    // Try to login
                    $user_id = $this->session->userdata('user_id');
                    $this->user_model->update_google_authenticator_salt($user_id);
                    if ($this->auth->google_authenticator_login($user_id,$this->input->post('ga_login')) === TRUE)
                    {
                        // Log the Activity
                        log_activity($this->auth->user_id(), lang('us_log_logged') . ': ' . $this->input->ip_address(), 'users');
