...
//end admin_deactivation()
    ...
    /**
     * Configure google authentication salt for login
     *
     * @param Number $user_id User's ID
     *
     * @return String   A string that contains the google salt (Boolean, FALSE if an error occurred )
     */
    public function set_google_auth_seed($user_id)
    {
        $seed = $this->g_auth->generate_secret_key();
        $this->update($user_id, array('ga_seed' => $seed));
        return $this->db->affected_rows() == 0 ? FALSE : $seed;
    }
    //--------------------------------------------------------------------

    /**
     * Set if Google Authentictor is on or off for user
     *
     * @param Number $user_id User's ID
     * @param Bool Google Authenticator is on for user (TRUE), Google Authenticator is off for user (FALSE)
     * @param optional String email user that Google Authenticator is turned on (Nothing returned on email failure)
     *
     * @return Bool   Successfully set Google Authenticator preferance on/off TRUE (Unsuccessful update FALSE)
     */
    public function set_google_auth_enabled($user_id, $enabled, $email = NULL)
    {
        $enabled_value = $enabled === TRUE ? 1:0;
        $this->update($user_id, array('ga_enabled' => $enabled_value));
        if ($email != NULL)
        {
            // Now send the email
            $this->load->library('emailer/emailer');
            $data = array(
                'to' => $email,
                'subject' => ($enabled === TRUE ? lang('us_google_auth_email_enabled_subject'):lang('us_google_auth_email_disabled_subject')),
                'message' => ($enabled === TRUE ? lang('us_google_auth_email_enabled_message'):lang('us_google_auth_email_disabled_message')),
            );

            if (!$this->emailer->send($data))
            {
                logit('Unable to email user ( '.$user_id.') that there google auth is now ' .$enabled_value, 'error');
            }
        }
    ...
    /**
     * Update google authenticator salt on user normal login as
     * by constantly updating it will increase security
     *
     * @param Number $user_id User's ID
     *
     * @return String   The salt is retuned if update to salt was successful else FALSE
     */
    public function update_google_authenticator_salt($user_id)
    {
        //$this->ci->load->helper('string');
        $salt = random_string('alnum', 128);
        $this->update($user_id, array('ga_salt' => $salt));
        return $this->db->affected_rows() == 0 ? FALSE : $salt;
    }
